...
> 版本：v2（极光 · 淘宝橙）｜适用：`frontend/`（React 19 + Ant Design v6 + Vite）
> 本文件取代旧版「Apple-design-analysis」（与产品不匹配）。

---

## 一、品牌方向

- **主色**：淘宝橙 `#ff7a1f`（浅/深一致），强调色渐变 `#ffb061 → #ff5a1f`。
- **氛围**：极光玻璃——近黑深色底 + 半透明面板 + 橙/蓝极光径向渐变背景。
- **默认主题**：深色（`DEFAULT_MODE = "dark"`），顶部栏提供「浅色 ⇄ 深色」一键切换，用户选择会持久化到 `localStorage("tb-workbench-theme")`。
- **语义色**：涨=绿、跌=红、警告=橙，仅用于增量/状态，不与分类色混用。
...
**规则**：
1. 组件里一律用 `var(--ops-*)`，**不要写死 hex/rgba**（旧代码里的写死颜色正在逐步清理）。
...
- 分类色板（`--ops-cat-0..5`）：橙/蓝/紫/绿/青/粉，用于多系列/漏斗/榜单；**不超过 6 色**。
...
- 圆角阶梯：`sm=8`（输入/表内）→ `md=12`（卡片内嵌）→ `lg=16`（卡片）→ `pill=999`（按钮/胶囊）。**不要再引入 10/11/14/21 等游离值**（存量代码逐步归并）。
- 正文基准 `14px`（AntD），页面大标题可用 `600` 字重；KPI 大数字允许 `700`。
- 字族：`Inter → PingFang SC → Microsoft YaHei → system-ui`。

## 五、组件规范

- **卡片**：`--ops-panel` 底 + `1px var(--ops-border)` 发丝边框 + `lg` 圆角；数据密集页可加 `--ops-shadow-sm` 轻阴影提升扫描性。
- **按钮**：主按钮 = 橙胶囊（`ant-btn-primary`）；次按钮 = 中性描边 + 橙字。
...
- 主色橙 `#ff7a1f` 在深底上对比度良好；浅色场景用于大号文字/图形，小号灰字用 `--ops-text-2/3`。
...
4. 品牌名/Logo/主色（用户可配）→ `lib/brand.ts` + 后端 `settings.py` 的 `DEFAULT_BRAND`。
