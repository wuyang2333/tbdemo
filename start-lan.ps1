# start-lan.ps1 - 局域网一键启动：后端 0.0.0.0:8000 + 前端 0.0.0.0:5173
...
if (Test-Port 8000) {
  Write-Host '[提示] 后端已在运行 (8000)，跳过启动'
} else {
  Write-Host '[1/3] 启动后端 (0.0.0.0:8000) ...'
  Start-Process -FilePath $PY -ArgumentList '-m','uvicorn','backend.app.main:app','--reload','--host','0.0.0.0','--port','8000' -WorkingDirectory $Root -WindowStyle Hidden -RedirectStandardOutput (Join-Path $Root 'logs\backend.log') -RedirectStandardError (Join-Path $Root 'logs\backend.err.log')
}

if (Test-Port 5173) {
  Write-Host '[提示] 前端已在运行 (5173)，跳过启动'
} else {
  Write-Host '[2/3] 启动前端 (0.0.0.0:5173) ...'
